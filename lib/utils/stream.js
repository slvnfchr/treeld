...
import { createChunk, Registry } from '../deserializer.js';
...
const enqueue = (chunk, controller) => {
	const registry = chunk['@registry'];
	Object.keys(chunk)
		.filter((key) => key !== '@parent')
		.forEach((key) => {
			if (Object.prototype.toString.call(chunk[key]) === '[object Object]') {
				chunk[key] = createChunk(registry, chunk[key]);
				chunk[key]['@parent'] = chunk;
				chunk[key]['@type'] = key;
				enqueue(chunk[key], controller);
			} else if (Array.isArray(chunk[key])) {
				const items = chunk[key].filter((c) => Object.prototype.toString.call(c) === '[object Object]');
				items.forEach((c, index) => {
					items[index] = createChunk(registry, c);
					items[index]['@parent'] = chunk;
					items[index]['@type'] = key;
					items[index]['@index'] = index;
					enqueue(items[index], controller);
				});
			}
		});
	controller.enqueue(chunk);
...
export const createStreamFromObject = (obj) => {
	...
	const stream = new ReadableStream({
		start(controller) {
			chunks.forEach((chunk) => enqueue(createChunk(registry, chunk), controller));
			controller.close();
		},
	});
	return stream;
