...
import registry from './deserializer/registry.spec.js';

const SAMPLE = path.resolve(process.cwd(), 'samples/smallest.ged');

describe('Deserializer', () => {
	describe('Registry utilities', registry);
	...
	it('Emitted chunk for simple object should have expected properties', async () => {
		...
		for await (const obj of data.pipeThrough(parser).pipeThrough(deserializer)) {
			...
			expect(obj).to.have.all.keys(property);
	...
	it('Emitted chunks for object with nested hierarchy should have expected properties', async () => {
		...
		expect(child).to.have.all.keys(subproperty);
		expect(child['@parent']).to.equal(parent);
		expect(child['@type']).to.equal(property1);
		expect(child[subproperty]).to.equal(value1);
		expect(parent).to.have.all.keys(property1, property2);
		expect(parent['@type']).to.equal(tag);
		expect(parent[property1]).to.equal(child);
		expect(parent[property2]).to.equal(value2);
	});

	it('Emitted chunks for object with duplicate scalar properties should have expected ones', async () => {
		const level = 0;
		const tag = 'TAG';
		const property = 'PROPERTY';
		const value1 = 'VALUE1';
		const value2 = 'VALUE2';
		const data = createStreamFromData([`${level} ${tag}`, `${level + 1} ${property} ${value1}`, `${level + 1} ${property} ${value2}`]);
		const parser = new Parser();
		const deserializer = new Deserializer();
		for await (const obj of data.pipeThrough(parser).pipeThrough(deserializer)) {
			expect(obj['@type']).to.equal(tag);
			...
			expect(obj[property][1]).to.equal(value2);
		}
	});

	it('Emitted chunks for object with duplicate object properties should have expected ones', async () => {
		const level = 0;
		const tag = 'TAG';
		const property = 'PROPERTY';
		const subproperty = 'SUBPROPERTY';
		...
		const data = createStreamFromData([
			`${level} ${tag}`,
			`${level + 1} ${property} ${value1}`,
			`${level + 2} ${subproperty} ${value2}`,
			`${level + 1} ${property} ${value3}`,
			`${level + 2} ${subproperty} ${value4}`,
		]);
		const parser = new Parser();
		const deserializer = new Deserializer();
		const chunks = [];
		...
		const [child1, child2, parent] = chunks;
		expect(parent['@type']).to.equal(tag);
		expect(parent).to.have.property(property).to.be.an('array');
		expect(parent[property][0]).to.equal(child1);
		expect(parent[property][0]['@index']).to.equal(0);
		expect(parent[property][1]).to.equal(child2);
		expect(parent[property][1]['@index']).to.equal(1);
	});
	...
	it('Emitted chunks for several objects can be retrieved through chunks promise', async () => {
		const tags = [1, 2, 3];
		const data = createStreamFromData(tags.reduce((lines, i) => lines.concat([`0 tag${i}`, `1 property${i} value${i}`]), []));
		const parser = new Parser();
		const deserializer = new Deserializer();
		data.pipeThrough(parser).pipeThrough(deserializer);
		const chunks = await deserializer.chunks();
		expect(chunks.length).to.equal(tags.length);
	});
	...
	it('Emitted chunks cross-references should be resolved when referenced record is previously emitted', async () => {
		...
		const property = 'PROPERTY';
		const value = 'VALUE';
		const data = createStreamFromData([`${level} ${ref} ${tag1}`, `${level + 1} ${property} ${value}`, `${level} ${tag2}`, `${level + 1} ${property} ${ref}`]);
		const parser = new Parser();
		const deserializer = new Deserializer();
		data.pipeThrough(parser).pipeThrough(deserializer);
		const chunks = await deserializer.chunks();
		expect(chunks[1][property]).to.equal(chunks[0]);
	});
	...
	it('Emitted chunks cross-references should be resolved when referenced record is not previously emitted', async () => {
		...
		const property = 'PROPERTY';
		const value = 'VALUE';
		const data = createStreamFromData([`${level} ${tag2}`, `${level + 1} ${property} ${ref}`, `${level} ${ref} ${tag1}`, `${level + 1} ${property} ${value}`]);
		const parser = new Parser();
		const deserializer = new Deserializer();
		data.pipeThrough(parser).pipeThrough(deserializer);
		const chunks = await deserializer.chunks();
		expect(chunks[1]).to.equal(chunks[0][property]);
	});
});
