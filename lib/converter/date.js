...
/**
 * GEDCOM date record conversion module
 * @module
 */
...
const DATE = String.raw`((${Object.values(CALENDAR).map((c) => c.escape).join('|')})\s)?(([0-9]{1,2})\s)?(([A-Z]{3,5})\s)?([0-9]+)(\S)?`;
...
export const parse = (str) => {
	...
	if (PERIOD_RE.test(str)) {
		...
		const start = parts[2];
		const end = parts[2 + DATE_RE_PARTS + 2];
