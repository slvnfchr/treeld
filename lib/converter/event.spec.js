...
import { isObject, bio, rdfs } from './context';
import Converter from '../converter';

export default () => {

	it('Event attribute with known type should be converted to a JSON-LD objet with corresponding type', (done) => {
	...
	it('Event attribute with unknown type should be converted to a JSON-LD objet with additional type annotation', (done) => {
		const date = '1841';
		const place = 'London, United Kingdom';
		const event = { DATE: date, PLAC: place };
		...
		const onData = spy(({ data: obj }) => {
			...
			} else { // Additional stream chunk for event type
	...
	it('Custom event attribute should be converted to a JSON-LD objet with additional comment property', (done) => {
		const date = '1816';
