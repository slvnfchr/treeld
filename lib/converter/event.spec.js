...
import { ASSOCIATION, EVENT, MULTIMEDIA, INDIVIDUAL, ROLE_TYPES, SHARED_NOTE, SOURCE, QUALITY } from '../gedcom/constants.js';
import { isObject, bio, schema, rdfs, skos, foaf } from './context.js';
...
export default () => {
	...
	it('Event object with age should be converted to a JSON-LD objet with foaf:age property', async () => {
		const age = '18y';
		const comment = 'this is a comment';
		const src = { '@type': 'DEAT', 'AGE': age };
		const data = createStreamFromObject(src);
		const converter = new Converter();
		for await (const obj of data.pipeThrough(converter)) {
			expect(isObject(obj[`${foaf.PREFIX}:age`], schema.TYPES.DURATION)).to.be.true;
			expect(obj[`${foaf.PREFIX}:age`].value).to.equal('P18Y');
		}
	});

	it('Even record with cause should have expected properties', async () => {
		const cause = 'Accident';
		const src = { '@type': 'DEAT', 'CAUS': cause };
		const data = createStreamFromObject(src);
		const converter = new Converter();
		for await (const obj of data.pipeThrough(converter)) {
			expect(obj).to.have.property('wdt:P828');
			expect(obj['wdt:P828']).to.equal(cause);
		}
	});
	...
	it('Event record with simple note should be converted to Event with a comment property', async () => {
		const note = 'Note related to event';
		const src = { '@type': EVENT, 'NOTE': note };
		const data = createStreamFromObject(src);
		const converter = new Converter();
		for await (const obj of data.pipeThrough(converter)) {
	...
	it('Event record with shared note should be converted to Event with a note property', async () => {
		const id = '@ID@';
		const note = 'Note related to event';
		const data = createStreamFromObject([
			{ '@ref': id, '@type': SHARED_NOTE, '@value': note },
			{ '@type': EVENT, [SHARED_NOTE]: id },
		]);
		const converter = new Converter();
		data.pipeThrough(converter);
		const [sharedNote, event] = await converter.chunks();
		expect(event).to.have.property(`${skos.PREFIX}:note`);
		expect(isObject(event[`${skos.PREFIX}:note`], schema.TYPES.CREATIVE_WORK)).to.be.true;
		expect(event[`${skos.PREFIX}:note`]).to.equal(sharedNote);
	});
