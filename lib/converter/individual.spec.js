...
import { isObject, schema, rdfs, bio, foaf, skos, rel } from './context.js';
...
export default () => {
	...
	it('Person record with title should have expected properties', async () => {
		const name = 'Augusta Ada /Byron/';
		const title = 'Countess of Lovelace';
		const src = { '@type': INDIVIDUAL, 'NAME': name, 'TITL': title };
		const data = createStreamFromObject(src);
		const converter = new Converter();
		for await (const obj of data.pipeThrough(converter)) {
			expect(obj).to.have.property('wdt:P97');
			expect(obj['wdt:P97']).to.equal(title);
		}
	});
	...
	it('Person record with association structure should have expected properties', async () => {
		const association = [
			{ 'ROLE': 'MOTH', '@value': '@1@' },
			{ 'ROLE': 'FATH', '@value': '@2@' },
			{ 'ROLE': 'CHIL', '@value': '@4@' },
			{ 'ROLE': { '@value': 'OTHER', 'PHRASE': 'Teacher' }, '@value': '@5@' },
		];
		const src = [
			{ '@ref': '@1@', '@type': INDIVIDUAL, 'NAME': 'Anne Isabella /Milbanke/' },
			{ '@ref': '@2@', '@type': INDIVIDUAL, 'NAME': 'Lord /Byron/' },
			{ '@ref': '@3@', '@type': INDIVIDUAL, 'NAME': 'Augusta Ada /Byron/', 'ASSO': association },
			{ '@ref': '@4@', '@type': INDIVIDUAL, 'NAME': 'Byron /King-Noel/' },
			{ '@ref': '@5@', '@type': INDIVIDUAL, 'NAME': 'Augustus /De Morgan/' },
		];
		const data = createStreamFromObject(src);
		const converter = new Converter();
		data.pipeThrough(converter);
		const [anne, lord, ada, byron] = await converter.chunks();
		expect(ada).to.have.property(`${rel.PREFIX}:childOf`);
		expect(ada[`${rel.PREFIX}:childOf`][0]).to.have.property('description', 'Mother');
		expect(ada[`${rel.PREFIX}:childOf`][0]).to.have.property('@value', '@1@');
		expect(ada[`${rel.PREFIX}:childOf`][1]).to.have.property('description', 'Father');
		expect(ada[`${rel.PREFIX}:childOf`][1]).to.have.property('@value', '@2@');
		expect(ada).to.have.property(`${rel.PREFIX}:parentOf`);
		expect(ada[`${rel.PREFIX}:parentOf`]).to.have.property('description', 'Child');
		expect(ada[`${rel.PREFIX}:parentOf`]).to.have.property('@value', '@4@');
		expect(ada).to.have.property(`${rel.PREFIX}:relationship`);
		expect(ada[`${rel.PREFIX}:relationship`]).to.have.property('description', 'Teacher');
		expect(ada[`${rel.PREFIX}:relationship`]).to.have.property('@value', '@5@');
	});
