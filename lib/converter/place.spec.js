...
import { PLACE, SHARED_NOTE } from '../gedcom/constants.js';
...
export default () => {
	...
	it('Place attribute with simple note should be converted to Place with a comment property', async () => {
		const address = 'Dorset street';
		const note = 'Note related to place';
		const src = { '@type': PLACE, '@value': address, 'NOTE': note };
		const data = createStreamFromObject(src);
		const converter = new Converter();
		for await (const obj of data.pipeThrough(converter)) {
	...
	it('Place attribute with shared note should be converted to Place with a note property', async () => {
		const address = 'Dorset street';
		const id = '@ID@';
		const note = 'Note related to place';
		const data = createStreamFromObject([
			{ '@ref': id, '@type': SHARED_NOTE, '@value': note },
			{ '@type': PLACE, '@value': address, [SHARED_NOTE]: id },
		]);
		const converter = new Converter();
		data.pipeThrough(converter);
		const [sharedNote, place] = await converter.chunks();
		expect(place).to.have.property(`${skos.PREFIX}:note`);
		expect(isObject(place[`${skos.PREFIX}:note`], schema.TYPES.CREATIVE_WORK)).to.be.true;
		expect(place[`${skos.PREFIX}:note`]).to.equal(sharedNote);
	});
};
