import { INDIVIDUAL, NAME, ROLE_TYPES } from '../gedcom/constants.js';
import { createObject, schema, foaf, bio, rdfs, dc, skos, wdt, rel } from './context.js';
...
const converter = (obj) => {
	...
	const map = {
		...
		TITL: (val) => {
			value.context.addVocabulary(wdt);
			value[`${wdt.PREFIX}:P97`] = val;
		},
		ASSO: (val) => {
			...
			const relationship = createObject(schema);
			relationship.type = schema.TYPES.PERSON;
			relationship.description = val.PHRASE || val.ROLE.PHRASE || ROLE_TYPES[roleType] || roleType;
			relationship['@value'] = val['@value'];
			if (val.NOTE) {
				relationship.context.addVocabulary(rdfs);
				relationship[`${rdfs.PREFIX}:comment`] = val;
			}
			if (val.SNOTE) {
			...
			if (val.SOUR) {
				relationship.subjectOf = val.SOUR;
			}
			value[`${rel.PREFIX}:${relationships[roleType] || 'relationship'}`] = relationship;
		},
		FAMS: (val) => {
			if (val.HUSB === value) {
