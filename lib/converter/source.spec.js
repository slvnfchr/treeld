...
export default () => {
	...
	it('Source record', async () => {
		...
		for await (const obj of data.pipeThrough(converter)) {
			expect(isObject(obj)).to.be.true;
			expect(obj).to.have.all.keys('@context', '@id', '@type', 'author', 'title', 'alternative', 'publisher', 'description', 'collection', 'modified', 'identifier');
			...
			expect(obj.collection).to.equal(repository);
