...
import { createObject, bio, rdfs, schema, skos, foaf, wdt } from './context.js';
import { mapObject } from './utils.js';
import { converter as dateConverter } from './date.js';
import { converter as placeConverter } from './place.js';
import { converter as ageConverter } from './age.js';
...
const converter = (obj, controller) => {
	...
	const map = {
		...
		'AGE': (val) => {
			...
			value[`${foaf.PREFIX}:age`] = typeof val === 'string' ? ageConverter(val).value : val;
		},
		'CAUS': (val) => {
			value.context.addVocabulary(wdt);
			value[`${wdt.PREFIX}:P828`] = val;
		},
		'ASSO': (val) => {
