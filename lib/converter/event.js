...
import { parse as parseLocation } from './place/index.js';
...
const converter = (obj, controller) => {
	...
	if (individual || group || obj.TYPE) {
		...
		// Date
		if (obj.DATE) {
			event.date = (typeof obj.DATE === 'string' && parseDate(obj.DATE)?.toISOString()) || obj.DATE;
		}
		// Date
		if (obj.PLAC) {
			event.place = typeof obj.PLAC === 'string' ? parseLocation(obj.PLAC) : obj.PLAC;
		}
		// Family link
		if (event.FAMC) {
