import { createObject, bio, rdfs } from './context.js';
// import { parse } from './event/age';
...
Object.values(TYPES_INDIVIDUAL)
 ...
 * @param {TransformStreamDefaultController} controller The transform stream controller used to emit additional chunk if needed
 * @returns {Object} The data object with the transformed property if applicable
 * @instance
 */

export default (data, property, controller) => {
	...
	if (individual) {
		if (typeof individual === 'string') {
			...
			controller.enqueue({ data: individual });
