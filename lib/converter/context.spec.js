...
export default () => {
	it('createObject should create an empty JSON-LD object in given context', (t, done) => {
	...
	it('createObject should enhance an given object into a JSON-LD object in given context', (t, done) => {
		expect(createObject).to.be.instanceof(Function);
	...
	it('JSON-LD object context can be manipulated by adding new vocabulary, removing existing one or change default one with properties name automatic change', (t, done) => {
	...
	it('Created object type can be set through type non-enumerable property which is serialized to @type', (t, done) => {
		expect(createObject).to.be.instanceof(Function);
		const obj = createObject(schema);
		const type = 'type';
		obj.type = type;
	...
	it('isObject should test if object is a JSON-LD object', (t, done) => {
	...
	it('mapProperties should rename several properties and merge them into array if applicable', (t, done) => {
	...
	it('flatten should apply properties subproperties to object', (t, done) => {
