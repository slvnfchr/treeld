...
import { MULTIMEDIA, RESTRICTION, SOURCE, QUALITY, SHARED_NOTE } from '../gedcom/constants.js';
...
export default () => {
	...
	it('Multimedia record with simple note should be converted to DigitalDocument with a comment property', async () => {
		const note = 'Taken during summer vacation';
		const src = { '@type': MULTIMEDIA, 'NOTE': note };
		const data = createStreamFromObject(src);
		const converter = new Converter();
		for await (const obj of data.pipeThrough(converter)) {
	...
	it('Multimedia record with shared note should be converted to DigitalDocument with a note property', async () => {
		const id = '@ID@';
		const note = 'Taken during summer vacation';
		const data = createStreamFromObject([
			{ '@ref': id, '@type': SHARED_NOTE, '@value': note },
			{ '@type': MULTIMEDIA, [SHARED_NOTE]: id },
		]);
		const converter = new Converter();
		data.pipeThrough(converter);
		const [sharedNote, multimedia] = await converter.chunks();
		expect(multimedia).to.have.property(`${skos.PREFIX}:note`);
		expect(isObject(multimedia[`${skos.PREFIX}:note`], schema.TYPES.CREATIVE_WORK)).to.be.true;
		expect(multimedia[`${skos.PREFIX}:note`]).to.equal(sharedNote);
	});
